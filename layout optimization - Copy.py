...
def load_furniture_proximity_matrix_csv(filepath):
    import csv
    proximity = {}
    with open(filepath, newline='') as f:
        reader = csv.reader(f)
        headers = next(reader)[1:]  # Skip first cell
        for row in reader:
            row_id = row[0]
            for col_id, val in zip(headers, row[1:]):
                try:
                    val = float(val)
                    if val > 0:
                        proximity[(row_id.strip(), col_id.strip())] = val
                except ValueError:
                    continue  # skip blanks or invalids
    return proximity

def load_room_proximity_matrix_csv(filepath):
    proximity = {}
    with open(filepath, newline='') as f:
        reader = csv.reader(f)
...
def proximity_penalty(rooms, proximity_matrix, travel_lines, room_names):
    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            prox = proximity_matrix.get((r1, r2)) or proximity_matrix.get((r2, r1))
            if prox is not None:
                line = travel_lines.get((r1, r2)) or travel_lines.get((r2, r1))
                if line is not None:
                    diffs.append(abs(prox - line.length))
...
def furniture_proximity_penalty(furniture_list, travel_lines, proximity_dict):
    total_penalty = 0.0
    count = 0
    for f1 in furniture_list:
        if not hasattr(f1, 'room') or f1.room is None:
            continue
        id1 = f"{f1.room.room_name}:{f1.furniture_name}"
        for f2 in furniture_list:
            if f1 == f2 or not hasattr(f2, 'room') or f2.room is None:
                continue
            id2 = f"{f2.room.room_name}:{f2.furniture_name}"
            prox = proximity_dict.get((id1, id2)) or proximity_dict.get((id2, id1))
            if prox is not None:
                line = travel_lines.get((f1, f2)) or travel_lines.get((f2, f1))
                if line:
                    total_penalty += abs(prox - line.length)
                    count += 1
    return total_penalty / count if count else 0.0

def overlap_penalty(rooms):
    penalty = 0.0
    for i, r1 in enumerate(rooms):
        for j, r2 in enumerate(rooms):
            if i < j and r1.offset_polygon and r2.offset_polygon:
                if r1.offset_polygon.intersects(r2.offset_polygon):
                    area = r1.offset_polygon.intersection(r2.offset_polygon).area
                    penalty += area * 1000  # Weighted penalty
    return penalty

def combined_objective(rooms, travel_lines, room_names, proximity_matrix_rooms,
                       furniture_list, furniture_proximity_dict,
                       area_weight=1.0, travel_weight=1.0,
                       perimeter_weight=2.0, proximity_weight=1.0,
                       furniture_proximity_weight=1.0):
    
    # 1. Combine all room polygons into one shape
    room_polys = [r.offset_polygon for r in rooms if r.offset_polygon]
    merged = unary_union(room_polys)

    # 2. Compute the perimeter of the unified shape
    perimeter = merged.length if merged else 0

    # 3. Area of rooms
    area = sum(r.offset_polygon.area for r in rooms if r.offset_polygon)

    # 4. Travel line length
    travel = total_travel_line_length(travel_lines)

    # 5. Proximity penalties
    room_prox = proximity_penalty(rooms, proximity_matrix_rooms, travel_lines, room_names)
    furniture_prox = furniture_proximity_penalty(furniture_list, travel_lines, furniture_proximity_dict)
    
    return (
        area_weight * area +
        travel_weight * travel +
        perimeter_weight * perimeter +
        proximity_weight * room_prox +
        furniture_proximity_weight * furniture_prox + overlap_penalty(rooms)
...
def run_optimization(rooms, travel_lines, furniture_list, desired_angles_per_room,
                     room_names, proximity_matrix_rooms, furniture_proximity_dict):
    def objective(x):
...
def main():
    ...
    furniture_proximity_dict = load_furniture_proximity_matrix_csv(
    r"C:\Users\user\Downloads\furniture_proximity_matrix.csv"
    )
    proximity_matrix_rooms = load_room_proximity_matrix_csv(
    r"C:\Users\user\Downloads\rooms_proximity_matrix.csv"
)
    # 1. Load CSV Data
